...
$MaxParallelMoves = 1 # Максимальное количество параллельных перемещений
$BadItemLimit = 100 # Лимит плохих элементов при миграции
$LogFolderPath = "C:\Scripts\Migration\Logs" # Путь для логов
$LogFile = "$LogFolderPath\MailboxMigration_OnlyArchive$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
        ...
        try {
            ...
            if ($existingRequest) {
                ...
                if ($existingRequest.Status -eq "InProgress" -or $existingRequest.Status -eq "Queued") {
                    ...
                    New-MoveRequest -Identity $Email -DomainController $DomainController -ArchiveOnly -ArchiveTargetDatabase $TargetDatabase -BadItemLimit $BadItemLimit -ErrorAction Stop
